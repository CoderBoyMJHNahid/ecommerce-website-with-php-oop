...
class Database{
		...
		// private $db_host = "localhost";  // Change as required
		// private $db_user = "wupov34zeldswe14_shooping_user";       // Change as required
		// private $db_pass = "W!DV8NygCDbb";   // Change as required
		// private $db_name = "wupov34zeldswe14_shopping";   // Change as required
		...
		public $hostname = "http://localhost/shopping/";
  ...
  public function pagination($table, $join = null, $where = null, $limit){
	...
	public function formatPrice($data){
		$formattedPrice = number_format($data, 0, '.', ',');

    	return $formattedPrice;
	}

	// position changing function
	public function changePosition($start,$old,$id){

		if ($start > $old) {
			$this->select("product","p_id,serial_num",null,"serial_num <>'{$id}'","serial_num ASC",$start);
			$result = $this->getResult();
			$i = 1;
			foreach ($result as $key => $value) {
				$this->update("product",['serial_num' => $i ],"p_id = '{$value['p_id']}'");
				$i++;
			}
		} else {
			$this->select("product","p_id,serial_num",null,"serial_num >= '{$start}'");
			$result = $this->getResult();
			foreach ($result as $key => $value) {
				$this->update("product",['serial_num' => $value['serial_num']+1 ],"p_id = '{$value['p_id']}'");
			}
		}
